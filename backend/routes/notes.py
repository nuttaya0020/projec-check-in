...
# routes/notes.py
@notes.route('/notes/<note_id>/checkout', methods=['POST'])
@jwt_required()
def update_checkout(note_id):
    try:
        data = request.get_json()
        checkout_time = data.get('checkout')

        # ✅ หา Note ด้วย MongoEngine (ใช้ .objects(id=...) แทน)
        note = Note.objects(id=note_id).first()
        if not note:
            return jsonify({'error': 'Note not found'}), 404

        note.checkout = datetime.fromisoformat(checkout_time)
        note.save()

        return jsonify({'message': 'Checkout time updated'})
